...
ConVar	ai_disappear("ai_disappear", "1", FCVAR_NONE, "Makes AI disappear after a specified amount of time.");
ConVar	ai_disappear_time("ai_disappear_time", "60", FCVAR_NONE);
...
ConVar	sk_npc_head( "sk_npc_head","3" );
...
void CAI_BaseNPC::FireBullets( const FireBulletsInfo_t &info )
{
	...
	if (FClassnameIs(this, "npc_combine_ace"))
	{
		modinfo.m_flDamage = (pWeapon->GetHL2MPWpnData().m_iPlayerDamage * 2);
		modinfo.m_iPlayerDamage = (pWeapon->GetHL2MPWpnData().m_iPlayerDamage * 2);
	}
	else
	{
		modinfo.m_flDamage = pWeapon->GetHL2MPWpnData().m_iPlayerDamage;
		modinfo.m_iPlayerDamage = pWeapon->GetHL2MPWpnData().m_iPlayerDamage;
	}
