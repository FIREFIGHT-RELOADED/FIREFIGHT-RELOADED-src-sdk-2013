...
#define HUD_LEVELDISPLAY_MINHIDETIME 7.0f

//-----------------------------------------------------------------------------
// Purpose: Displays small key-centric hints on the right hand side of the screen
//-----------------------------------------------------------------------------
class CHudLevelHintDisplay : public vgui::Panel, public CHudElement
{
	DECLARE_CLASS_SIMPLE(CHudLevelHintDisplay, vgui::Panel);

public:
	CHudLevelHintDisplay(const char *pElementName);
	void Init();
	void Reset();
	void MsgFunc_LevelHintText(bf_read &msg);
	bool ShouldDraw();
...
DECLARE_HUDELEMENT(CHudLevelHintDisplay);
DECLARE_HUD_MESSAGE(CHudLevelHintDisplay, LevelHintText);

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
CHudLevelHintDisplay::CHudLevelHintDisplay(const char *pElementName) : BaseClass(NULL, "HudLevelHintDisplay"), CHudElement(pElementName)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHudLevelHintDisplay::Init()
{
	HOOK_HUD_MESSAGE(CHudLevelHintDisplay, LevelHintText);
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CHudLevelHintDisplay::Reset()
{
	SetHintText( NULL );
	SetAlpha( 0 );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHudLevelHintDisplay::ApplySchemeSettings(vgui::IScheme *pScheme)
{
...
//-----------------------------------------------------------------------------
bool CHudLevelHintDisplay::ShouldDraw(void)
{
...
//-----------------------------------------------------------------------------
void CHudLevelHintDisplay::OnTick(void)
{
...
//-----------------------------------------------------------------------------
void CHudLevelHintDisplay::OnThink()
{
...
//-----------------------------------------------------------------------------
bool CHudLevelHintDisplay::SetHintText(const char *text)
{
	...
	m_flHudHintMinDisplayTime = gpGlobals->curtime + HUD_LEVELDISPLAY_MINHIDETIME;
...
//-----------------------------------------------------------------------------
void CHudLevelHintDisplay::MsgFunc_LevelHintText(bf_read &msg)
{
	...
	if ( count != 1 )
	{
		DevMsg("CHudLevelHintDisplay::MsgFunc_levelHintText: string count != 1.\n");
		return;
	}
	...
	{
		SetVisible( true );
 		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence( "LevelHintMessageShow" ); 
	}
	else
	{
		// it's being cleared, hide the panel
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence( "LevelHintMessageHide" ); 
	}
}
