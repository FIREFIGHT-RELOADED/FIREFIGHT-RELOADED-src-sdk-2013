...
void CNPC_CombineE::Spawn( void )
{
	...
	if (m_pAttributes != NULL)
	{
		...
		if (grenadeoverride && combine_guard_spawnwithgrenades.GetBool())
		{
...
void CNPC_CombineE::PrescheduleThink( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineE::SelectSchedule ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_CombineE::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	bool gibs = true;
	if (m_pAttributes != NULL)
	{
	...
	case HITGROUP_HEAD:
		if (!(g_Language.GetInt() == LANGUAGE_GERMAN || UTIL_IsLowViolence()) && g_fr_headshotgore.GetBool() && gibs)
		{
			...
			else if ((info.GetDamageType() & (DMG_SLASH)) && !(info.GetDamageType() & DMG_NEVERGIB))
			{
				...
				SpawnBlood(GetAbsOrigin(), g_vecAttackDir, BloodColor(), info.GetDamage());
				CBaseEntity* pHeadGib = CGib::SpawnSpecificSingleGib(this, 150, 450, "models/gibs/soldier_elite_head.mdl", 6);
...
void CNPC_CombineE::OnChangeActivity( Activity eNewActivity )
{
...
void CNPC_CombineE::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if (!(g_Language.GetInt() == LANGUAGE_GERMAN || UTIL_IsLowViolence()) && info.GetDamageType() & (DMG_BLAST | DMG_CRUSH) && !(info.GetDamageType() & (DMG_DISSOLVE)) && !PlayerHasMegaPhysCannon() && gibs)
	{
		...
		CBaseEntity* pHeadGib = CGib::SpawnSpecificSingleGib(this, 750, 1500, "models/gibs/soldier_elite_head.mdl", flFadeTime);
		...
		CBaseEntity *pLeftArmGib = CreateRagGib(this, "models/gibs/soldier_elite_left_arm.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pLeftArmGib)
		{
		...
		CBaseEntity *pRightArmGib = CreateRagGib(this, "models/gibs/soldier_elite_right_arm.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pRightArmGib)
		{
		...
		CBaseEntity *pTorsoGib = CreateRagGib(this, "models/gibs/soldier_elite_torso.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pTorsoGib)
		{
		...
		CBaseEntity *pPelvisGib = CreateRagGib(this, "models/gibs/soldier_elite_pelvis.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pPelvisGib)
		{
		...
		CBaseEntity *pLeftLegGib = CreateRagGib(this, "models/gibs/soldier_elite_left_leg.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pLeftLegGib)
		{
		...
		CBaseEntity *pRightLegGib = CreateRagGib(this, "models/gibs/soldier_elite_right_leg.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pRightLegGib)
		{
...
//-----------------------------------------------------------------------------
Activity CNPC_CombineE::NPC_TranslateActivity( Activity eNewActivity )
{
