...
void CNPC_CombineShot::Spawn(void)
{
	...
	if (m_pAttributes != NULL)
	{
		...
		if (grenadeoverride && combine_shot_spawnwithgrenades.GetBool())
		{
...
void CNPC_CombineShot::DeathSound(const CTakeDamageInfo &info)
{
...
void CNPC_CombineShot::PrescheduleThink(void)
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineShot::SelectSchedule(void)
{
	...
	case HITGROUP_HEAD:
		if (!(g_Language.GetInt() == LANGUAGE_GERMAN || UTIL_IsLowViolence()) && g_fr_headshotgore.GetBool() && gibs)
		{
			...
			else if ((info.GetDamageType() & (DMG_SLASH)) && !(info.GetDamageType() & DMG_NEVERGIB))
			{
				...
				SpawnBlood(GetAbsOrigin(), g_vecAttackDir, BloodColor(), info.GetDamage());
				CBaseEntity* pHeadGib = CGib::SpawnSpecificSingleGib(this, 150, 450, "models/gibs/soldier_shotgunner_head.mdl", 6);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineShot::HandleAnimEvent(animevent_t *pEvent)
{
	switch( pEvent->event )
	{
...
void CNPC_CombineShot::OnChangeActivity(Activity eNewActivity)
{
...
void CNPC_CombineShot::Event_Killed(const CTakeDamageInfo &info)
{
	...
	if (!(g_Language.GetInt() == LANGUAGE_GERMAN || UTIL_IsLowViolence()) && info.GetDamageType() & (DMG_BLAST | DMG_CRUSH) && !(info.GetDamageType() & (DMG_DISSOLVE)) && !PlayerHasMegaPhysCannon() && gibs)
	{
		...
		CBaseEntity* pHeadGib = CGib::SpawnSpecificSingleGib(this, 750, 1500, "models/gibs/soldier_shotgunner_head.mdl", flFadeTime);
		...
		CBaseEntity *pLeftArmGib = CreateRagGib(this, "models/gibs/soldier_shotgunner_left_arm.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pLeftArmGib)
		{
		...
		CBaseEntity *pRightArmGib = CreateRagGib(this, "models/gibs/soldier_shotgunner_right_arm.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pRightArmGib)
		{
		...
		CBaseEntity *pTorsoGib = CreateRagGib(this, "models/gibs/soldier_shotgunner_torso.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pTorsoGib)
		{
		...
		CBaseEntity *pPelvisGib = CreateRagGib(this, "models/gibs/soldier_shotgunner_pelvis.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pPelvisGib)
		{
		...
		CBaseEntity *pLeftLegGib = CreateRagGib(this, "models/gibs/soldier_shotgunner_left_leg.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pLeftLegGib)
		{
		...
		CBaseEntity *pRightLegGib = CreateRagGib(this, "models/gibs/soldier_shotgunner_right_leg.mdl", GetAbsOrigin(), GetAbsAngles(), vecRagDmgForce, flFadeTime, IsOnFire());
		if (pRightLegGib)
		{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_CombineShot::IsLightDamage(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
Activity CNPC_CombineShot::NPC_TranslateActivity(Activity eNewActivity)
{
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CNPC_CombineShot)
