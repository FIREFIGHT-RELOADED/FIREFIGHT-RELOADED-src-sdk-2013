...
ConVar	ociw_grenade_credit_transfer("ociw_grenade_credit_transfer", "1");
#endif
...
// Function pointers
DEFINE_ENTITYFUNC(GrenadeOCIWTouch),
DEFINE_THINKFUNC(GrenadeOCIWThink),

END_DATADESC()

LINK_ENTITY_TO_CLASS(grenade_ociw, CGrenadeOCIW);

void CGrenadeOCIW::Spawn(void)
{
	Precache();
	SetSolid(SOLID_BBOX);
	SetMoveType(MOVETYPE_FLYGRAVITY, MOVECOLLIDE_FLY_BOUNCE);
	...
	SetModel("models/Weapons/oicw_grenade.mdl");
	UTIL_SetSize(this, Vector(-3, -3, -3), Vector(3, 3, 3));
	//	UTIL_SetSize(this, Vector(0, 0, 0), Vector(0, 0, 0));

	SetUse(&CGrenadeOCIW::DetonateUse);
	SetTouch(&CGrenadeOCIW::GrenadeOCIWTouch);
	SetThink(&CGrenadeOCIW::GrenadeOCIWThink);
	SetNextThink(gpGlobals->curtime + 0.1f);
...
// Output :
//-----------------------------------------------------------------------------
void CGrenadeOCIW::GrenadeOCIWThink(void)
{
	SetNextThink(gpGlobals->curtime + 0.05f);
...
void CGrenadeOCIW::Event_Killed(const CTakeDamageInfo &info)
{
#ifdef MAPBASE
	if (ociw_grenade_credit_transfer.GetBool() && info.GetAttacker()->MyCombatCharacterPointer())
	{
		CBaseCombatCharacter *pBCC = info.GetAttacker()->MyCombatCharacterPointer();
		SetThrower(pBCC);
		SetOwnerEntity(pBCC);
...
void CGrenadeOCIW::GrenadeOCIWTouch(CBaseEntity *pOther)
{
	Assert(pOther);
	if (!pOther->IsSolid())
		return;
...
void CGrenadeOCIW::Detonate(void)
{
	if (!m_bIsLive)
	{
		return;
...
void CGrenadeOCIW::Precache(void)
{
	PrecacheModel("models/Weapons/oicw_grenade.mdl");
}


CGrenadeOCIW::CGrenadeOCIW(void)
{
	m_hSmokeTrail = NULL;
}
